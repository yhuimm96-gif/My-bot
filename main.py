    ...
    try:
        if os.path.exists(DB_NAME):
            with open(DB_NAME, 'rb') as doc:
                bot.send_document(CONFIG['ADMIN_ID'], doc, caption=f"📂 نسخة احتياطية\n📅 {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    except: pass
...
def user_actions(call):
    ...
    if call.data == 'u_bal': 
        ...
        text = (
            ...
            f"💰 تحصل على 1$ مكافأة فورية عن كل إحالة تقوم بالإيداع."
        )
        bot.send_message(call.message.chat.id, text, parse_mode='Markdown')
    elif call.data == 'u_dep':
        if user['has_deposited'] == 1:
            bot.answer_callback_query(call.id, "⚠️ عذراً، يسمح بالإيداع لمرة واحدة فقط!", show_alert=True)
            return
        markup = types.InlineKeyboardMarkup(row_width=1)
    ...
    if data[0] == 'app':
        ...
        if t_user['referred_by']:
            ...
            if ref:
                ...
                try: 
                    bot.send_message(ref['uid'], f"🎉 **مبروك! أحد إحالاتك أودع الآن.**\n✅ تمت إضافة مكافأة: `1$` لرصيدك.\n📈 إجمالي الإحالات الفعالة: `{new_act}`", parse_mode='Markdown')
                except: pass
        
        bot.send_message(t_uid, "✅ تم قبول إيداعك بنجاح! ستبدأ أرباحك بالنزول يومياً.")
    else: 
        update_user(t_uid, pending_amount=0)
        bot.send_message(t_uid, "❌ تم رفض الإثبات، يرجى التواصل مع الدعم.")
...
def final_wit(message, amt):
    ...
    if not address.startswith("0x") or len(address) != 42: 
        bot.send_message(message.chat.id, "❌ عنوان محفظة غير صالح.")
        return
    user = get_user(message.from_user.id)
    ...
    bot.send_message(message.chat.id, "⏳ طلبك قيد المراجعة من قبل الإدارة.")
