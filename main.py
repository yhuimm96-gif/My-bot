...
def edit_profit_step(call):
    ...
    msg = bot.send_message(call.message.chat.id, f"أدخل قيمة الربح اليومي الجديدة لباقة {package}$:")
...
def save_profit_setting(message, package):
    try:
        ...
        with get_db_connection() as conn:
            ...
            conn.commit()
        bot.send_message(message.chat.id, f"✅ تم تحديث ربح باقة {package}$ إلى {new_val}$.")
...
def start(message):
    ...
    if not user:
        ...
        # سجل دخول عضو جديد
        bot.send_message(CONFIG['ADMIN_ID'], f"🆕 **عضو جديد سجل في البوت:**\n👤 الاسم: {message.from_user.first_name}\n🆔 الآيدي: `{uid}`")
        user = get_user(uid)
...
def save_user_name(message):
    if message.text and len(message.text.split()) >= 3:
        ...
        msg = bot.send_message(message.chat.id, "⚠️ يرجى كتابة اسمك الثلاثي بشكل صحيح (3 كلمات على الأقل):")
        bot.register_next_step_handler(msg, save_user_name)
...
def user_actions(call):
    ...
    if call.data == 'u_bal': 
        bot.answer_callback_query(call.id, f"رصيدك الحالي: {user['balance']:.2f}$\nالمبلغ القابل للسحب: {user['withdrawable_profit']:.2f}$", show_alert=True)
        ...
        text = (
            ...
            f"✅ إحالات مستثمرة: `{user['active_referrals']}`\n\n"
            f"💰 مكافأة الإحالة: تحصل على 1$ عن كل شخص يودع من خلالك."
        )
        bot.send_message(call.message.chat.id, text, parse_mode='Markdown')
    elif call.data == 'u_dep':
        if user['has_deposited'] == 1:
            bot.answer_callback_query(call.id, "⚠️ عذراً، الإيداع مسموح به مرة واحدة فقط لكل مستخدم.", show_alert=True)
            return
        markup = types.InlineKeyboardMarkup(row_width=1)
        ...
        bot.edit_message_text("💰 اختر باقة الاستثمار المطلوبة:", call.message.chat.id, call.message.message_id, reply_markup=markup)
        ...
        if user['withdrawable_profit'] <= 0: 
            bot.answer_callback_query(call.id, "⚠️ رصيدك الحالي 0، لا يوجد ما يمكن سحبه.", show_alert=True)
            return
        msg = bot.send_message(call.message.chat.id, f"💵 أدخل المبلغ الذي تود سحبه (المتاح: {user['withdrawable_profit']:.2f}$):")
...
def package_select(call):
    ...
    bot.edit_message_text(f"✅ اخترت باقة {val}$\n\nيرجى تحويل المبلغ إلى عنوان الشبكة (BEP20):\n`{get_setting('wallet')}`\n\nبعد التحويل، أرسل صورة إثبات التحويل (Screenshot) هنا.", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
...
def handle_proof(message):
    ...
    bot.send_message(CONFIG['ADMIN_ID'], f"📩 **إشعار إيداع جديد**\n👤 المستثمر: {user['full_name']}\n💰 المبلغ المتوقع: {user['pending_amount']}$\n🆔 الآيدي: `{user['uid']}`", reply_markup=markup)
    bot.send_message(message.chat.id, "⏳ تم استلام الإثبات. جاري المراجعة من قبل الإدارة...")
    ...
    if data[0] == 'app':
        ...
        if t_user['referred_by']:
            ...
            if ref:
                ...
                try: bot.send_message(ref['uid'], f"🎉 مبروك! أحد إحالاتك قام بالإيداع.\n✅ تمت إضافة 1$ لمكافأتك.")
                except: pass
        
        bot.send_message(t_uid, "✅ تهانينا! تم تأكيد إيداعك وبدأ حساب الأرباح.")
        # سجل في الأدمن
        bot.send_message(CONFIG['ADMIN_ID'], f"✅ تم تفعيل حساب المستخدم {t_uid} بمبلغ {amt}$")
    else: 
        update_user(t_uid, pending_amount=0)
        bot.send_message(t_uid, "❌ تم رفض إثبات الإيداع. يرجى التواصل مع الدعم الفني.")
...
def process_withdraw(message):
    try:
        ...
        if amt > user['withdrawable_profit']: 
            bot.send_message(message.chat.id, "⚠️ المبلغ المدخل أكبر من رصيدك المتاح للسحب."); return
        if amt < 1: 
            bot.send_message(message.chat.id, "⚠️ الحد الأدنى للسحب هو 1$."); return
            
        msg = bot.send_message(message.chat.id, "💳 يرجى إرسال عنوان محفظتك (BEP20) لاستلام السحب:"); 
        bot.register_next_step_handler(msg, final_wit, amt)
    except: bot.send_message(message.chat.id, "⚠️ خطأ: يرجى إدخال مبلغ صحيح.")
...
def final_wit(message, amt):
    ...
    # إرسال طلب السحب للأدمن (سجل السحب)
    bot.send_message(CONFIG['ADMIN_ID'], f"📤 **طلب سحب جديد**\n👤 الاسم: {user['full_name']}\n💰 المبلغ: {amt}$\n💳 العنوان: `{address}`\n🆔 الآيدي: `{user['uid']}`")
    bot.send_message(message.chat.id, "⏳ تم إرسال طلب السحب. سيتم تحويل المبلغ خلال 24 ساعة.")

if __name__ == "__main__":
    print("البوت يعمل الآن بنظام السجلات المحدث...")
